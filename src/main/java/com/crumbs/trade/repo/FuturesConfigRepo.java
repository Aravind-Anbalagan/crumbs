...
public interface FuturesConfigRepo extends JpaRepository<FuturesConfig, Long> {
	...
	@Query("""
		    SELECT c
		    FROM FuturesConfig c
		    WHERE c.active = :active
		      AND c.indexType IN :indexTypes
		""")
		List<FuturesConfig> findByActiveAndIndexTypes(
		        @Param("active") String active,
		        @Param("indexTypes") List<String> indexTypes
		);
	...
	/**
	 * Find ALL configs by active status Use this to get all active configs
	 */
